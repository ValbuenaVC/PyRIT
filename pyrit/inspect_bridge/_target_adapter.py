...
``import pyrit`` and ``import pyrit.inspect_bridge`` never load this module
(the parent ``__init__`` resolves it lazily via PEP 562 ``__getattr__``), so
importing ``inspect_ai`` at the top of this file is safe -- it only happens
when the adapter is actually used.
"""
