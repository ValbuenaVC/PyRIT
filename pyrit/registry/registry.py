...
class Registry(ABC, Generic[T, MetadataT]):
    ...
    def _register_subclasses_in_package(self, *, package_name: str) -> int:
        """
        Register every concrete base-type subclass whose module lives under a package.

        Used by built-in discovery (``_discover``): enumerates the in-memory subclasses of
        ``_base_type()``, keeps those whose module is ``package_name`` or a submodule of
        it, names each via ``_get_registry_name``, and registers it.
